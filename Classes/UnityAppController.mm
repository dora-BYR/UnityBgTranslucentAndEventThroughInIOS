...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    CGRect rect = CGRectMake(100.0, 50.0, 240.0, 400.0); // 设定UnityView的窗口宽高和位置 by longhui 20220216
#if (PLATFORM_IOS && defined(__IPHONE_13_0)) || (PLATFORM_TVOS && defined(__TVOS_13_0))
    if (@available(iOS 13, tvOS 13, *))
    {
            _window = [[UIWindow alloc] initWithWindowScene: [self pickStartupWindowScene: application.connectedScenes]];
//        _window = [[UIWindow alloc] initWithFrame: rect]; // 设定UnityView的窗口宽高和位置 by longhui 20220216
    }else
#endif
    {
...
//        _window = [[UIWindow alloc] initWithFrame: rect]; // 设定UnityView的窗口宽高和位置 by longhui 20220216
    ...
    CGRect rect1 = CGRectMake(200.0, 200.0, 400.0, 400.0); // 无效？
//    _unityView = [self createUnityViewWithFrame: rect1]; // by longhui 20220215
    _unityView.layer.zPosition = 200;
    _unityView.tag = 1001;
    [_unityView setHidden: YES];
    
//    CGRect rect2 = CGRectMake(200.0, 200.0, 400.0, 400.0);
//    MyTempView *tempView = [[MyTempView alloc] initWithFrame: rect2];
//    [tempView addSubview: _unityView];
    
    // 一个在unity之上的原生UIView by longhui 20220216
//    CGRect rect3 = CGRectMake(100.0, 300.0, 120.0, 120.0);
    MyTempView *tempView3 = [MyTempView createWithFrame: [UIScreen mainScreen].bounds];
//    const CGFloat components[] = {1.0f, 0.0f, 0.0f, 1.0f};
//    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
//    CGColorRef color = CGColorCreate(colorSpace, components);
//    tempView3.layer.masksToBounds = YES;
//    tempView3.layer.cornerRadius=5;
//    tempView3.layer.backgroundColor = color;
//    tempView3.layer.zPosition = 100;
    [_window addSubview: tempView3];
    
    /*
    // 一个在unity之下的原生UIImageView by longhui 20220216
...
//    NSData *data = [NSData dataWithContentsOfFile: @"chatbg.png"];
//    UIImage * imageData = [UIImage imageWithData: data];
    UIImageView *tempView4 = [[UIImageView alloc] initWithImage: imageData];
    tempView4.frame = [UIScreen mainScreen].bounds;
//    tempView4.contentMode = UIViewContentModeScaleToFill;
//    tempView4.layer.zPosition = 100;
     [_window addSubview: tempView4];
     */
    
    [DisplayManager Initialize];
    _mainDisplay = [DisplayManager Instance].mainDisplay;
    [_mainDisplay createWithWindow: _window andView: _unityView];
//    [_mainDisplay createWithWindow: _window andView: tempView3];
//    [self.rootView addSubview: _unityView];
