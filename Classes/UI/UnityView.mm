...
#import "../Libraries/Plugins/iOS/OCUnityCall.h" // by longhui 20220218
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (hitView == self) {
        NSLog(@"UnityView:点击中自己：%f, %f", point.x, point.y);
        if (g_getIsUnityViewNeedSwallowEvent) {
            bool isNeed = g_getIsUnityViewNeedSwallowEvent(point.x * self.contentScaleFactor, (_surfaceSize.height - point.y) * self.contentScaleFactor);
            NSLog(@"UnityView是否需要吞噬事件：%i", isNeed);
//            NSLog(@"UnityView是否需要吞噬事件：%i", g_isNeedUnityViewSwallowEvent);
            if (isNeed) {
                return self;
            }
        }else {
            NSLog(@"UnityView:未找到设置函数");
        }
//        if (g_isNeedUnityViewSwallowEvent) {
//            return self;
//        }
        return nil;
    }else {
        NSLog(@"UnityView:未点击中自己");
    }
    return hitView;
}
